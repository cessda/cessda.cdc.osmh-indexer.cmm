...
public class HandlerConfigurationPropertiesTest {
  ...
  public void shouldReturnConfigurationsForOSMHHandler() {
    ...
    then(oaiPmh.getRepos().get(3).getUrl()).isEqualTo("https://kuha.sa.dk/v0/oai");
    then(oaiPmh.getRepos().get(3).getPreferredMetadataParam()).isEqualTo("oai_ddi25");
    then(oaiPmh.getRepos().get(4).getUrl()).isEqualTo("http://services.fsd.uta.fi/v0/oai");
    then(oaiPmh.getRepos().get(4).getPreferredMetadataParam()).isEqualTo("oai_ddi25");
    then(oaiPmh.getRepos().get(5).getUrl()).isEqualTo("https://dbk.gesis.org/dbkoai3/");
    then(oaiPmh.getRepos().get(5).getPreferredMetadataParam()).isEqualTo("oai_ddi25");
    then(oaiPmh.getRepos().get(6).getUrl()).isEqualTo("http://147.213.4.44:6003/v0/oai");
    then(oaiPmh.getRepos().get(6).getPreferredMetadataParam()).isEqualTo("oai_ddi25");
    then(oaiPmh.getRepos().get(7).getUrl()).isEqualTo("https://snd.gu.se/en/oai-pmh");
    then(oaiPmh.getRepos().get(7).getPreferredMetadataParam()).isEqualTo("oai_ddi25");
    then(oaiPmh.getRepos().get(8).getUrl()).isEqualTo("https://oai.ukdataservice.ac.uk:8443/oai/provider");
    then(oaiPmh.getRepos().get(8).getPreferredMetadataParam()).isEqualTo("ddi");
  }
}
