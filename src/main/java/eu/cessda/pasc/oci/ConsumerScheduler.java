...
public class ConsumerScheduler {
    ...
    public void runIndexer() {
        ...
        try (var _ = MDC.putCloseable(
            ...
            log.info("[" + FULL_RUN +"] Consume and Ingest All SPs Repos: \nStarted at [{}]\nCurrent state before run:\n{}",
                startTime,
            ...
            log.info("[" + FULL_RUN +"] Indexing started at [{}]", indexingStartTime);
            indexerRunner.executeHarvestAndIngest();

            final var indexingEndTime = OffsetDateTime.now(ZoneId.systemDefault());
            long durationSeconds = Duration.between(indexingStartTime, indexingEndTime).toSeconds();
            log.atInfo().setMessage("[" + FULL_RUN +"] Indexing completed at [{}] - Duration: [{}] seconds")
                    .addArgument(indexingEndTime)
                    .addArgument(durationSeconds)
                    .addKeyValue("indexing_duration", durationSeconds)
                    .log();

            // Perform reindexing
            final var reindexingStartTime = OffsetDateTime.now(ZoneId.systemDefault());
            log.info("[" + FULL_RUN + "] Thematic reindexing started at [{}]", reindexingStartTime);
            indexerRunner.executeReindexing();

            final var reindexingEndTime = OffsetDateTime.now(ZoneId.systemDefault());
            var reindexingDurationSeconds = Duration.between(reindexingStartTime, reindexingEndTime).toSeconds();
            log.atInfo().setMessage("[" + FULL_RUN +"] Thematic reindexing completed at [{}] - Duration: [{}] seconds")
                    .addArgument(reindexingEndTime)
                    .addArgument(reindexingDurationSeconds)
                    .addKeyValue("reindexing_duration", reindexingDurationSeconds)
                    .log();

            final var endTime = OffsetDateTime.now(ZoneId.systemDefault());
            var totalDurationSeconds = Duration.between(startTime, endTime).toSeconds();
            log.atInfo().setMessage("[" + FULL_RUN +"] Consume and Ingest All SPs Repos:\nEnded at: [{}]\nTotal Duration: [{}] seconds")
                    .addArgument(endTime)
                    .addArgument(totalDurationSeconds)
                    .addKeyValue("job_duration", totalDurationSeconds)
                    .log();
        } catch (IOException e) {
