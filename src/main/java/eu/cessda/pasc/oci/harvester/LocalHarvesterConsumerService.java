...
public class LocalHarvesterConsumerService extends AbstractHarvesterConsumerService {
    ...
    public Optional<CMMStudy> getRecordFromRemote(Repo repo, Record recordHeader) {
        try (var identifierClosable = MDC.putCloseable(LoggingConstants.STUDY_ID, recordHeader.getRecordHeader().getIdentifier())) {
            return Optional.of(recordXMLParser.getRecord(repo, recordHeader));
        } catch (OaiPmhException e) {
            e.getOaiErrorMessage().ifPresentOrElse(
